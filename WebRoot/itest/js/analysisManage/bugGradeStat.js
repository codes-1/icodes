...
$(function(){
	...
	//加载ehcarts
	var url = baseUrl + "/analysis/analysisAction!getBugGradeStat.action";
	$.post(
			url,
			{	"analysisDto.taskId":itemId
				...
				function(data){
					   if(data.length>0){
						   ...
						   loadEcharts('','bugGradeStatEcharts');
						   loadTable();
					   }
					},'json');
}); 
...
function loadTable(){
	  ...
	  document.getElementById("bugGradeStatTbody").innerHTML = tbody;
}
//@ sourceURL=bugGradeStat.js
