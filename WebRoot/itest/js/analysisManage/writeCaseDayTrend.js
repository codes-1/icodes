...
function getBugDate(id){
	var url = baseUrl + "/singleTestTask/singleTestTaskAction!getWriteCaseDateLimit.action";
	$.post(
			url,
...
function getData(itemId,startDate,endDate){
	var url = baseUrl + "/analysis/analysisAction!getWriteCaseDayTrend.action";
	$.post(
			url,
						...
						for(var a=0;a<nameDis.length;a++){
							...
							for(var b=0;b<dataDateDis.length;b++){
								...
								for(var c=0;c<data.length;c++){
									if(nameDis[a]==data[c][1]&&dataDateDis[b]==data[c][0]){
										lkl.push(data[c][2]);
										hjk = 1;
										break;
									}else{
...
//			}
	...
	var thead = "<th style=\"text-align:center\">日期/开发人员</th>";
	var tbody = "";
	var allData = [];

	
	for(var a=0;a<nameDis01.length;a++){
		thead +="<th style=\"text-align:center\">"+nameDis01[a]+"</th>";
		var lkl = [];
...
//@ sourceURL=writeCaseDayTrend.js
