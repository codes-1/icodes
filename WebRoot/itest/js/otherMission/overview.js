...
function searchOtherMission(){
	/*objs.$missionDg.xdatagrid({
		...
		queryParams: {
			...
			"dto.otherMission.chargePersonId":$("#accountId").text(),
		...
		columns:[[
			...
			//{field:'chargePersonId',title:'任务负责人',align:'center',halign:'center',formatter:chargePersonFormat},
			...
			{field:'status',title:'状态',align:'center',width:"8%",formatter:statusFormat},
	...
	$.post(baseUrl + "/otherMission/otherMissionAction!getOtherMissionList.action",{
		...
		if(data.length > 0){
			for(var i=0;i<data.length;i++){
				var missiName = "";
				if(data[i].missionName.length > 5){
					missiName = data[i].missionName.substring(0,5)+"...";
				}else{
				...
				if(data[i].status == "0"){
					...
					if(data[i].chargePersonId == $("#accountId").text()){
						if(data[i].missionNum){
							$("#distribution").append("<div style='margin-bottom:1em; text-align: left; padding-left: 10px;padding-top:0.5em'>"
									+"<a onclick='showDetail("+JSON.stringify(data[i])+")' style='cursor:pointer' title='"+data[i].missionName+"'>"+missiName+"</a><br/>"
					...
					if(data[i].chargePersonId == $("#accountId").text()){
						if(data[i].missionNum){
							$("#accept").append("<div style='margin-bottom:1em; text-align: left; padding-left: 10px;padding-top:0.5em'>"
									+"<a onclick='showDetail("+JSON.stringify(data[i])+")' style='cursor:pointer;margin-right:3px' title='"+data[i].missionName+"'>"+missiName+"</a>("+data[i].completionDegree+"%)<br/>"
									+"<span style='margin-right:5px;'>#"+data[i].missionNum+"</span><span>"+charPerson+"</span><a onclick='showEditWin("+JSON.stringify(data[i])+")' style='cursor:pointer;margin-left: 50px;'>  填写进度</a>"
									+"</div>");
						}else{
							$("#accept").append("<div style='margin-bottom:1em; text-align: left; padding-left: 10px;padding-top:0.5em'>"
...
function operationFormat(value,row,index){
	if(row.status != "3"){
		var html = "<a style='cursor:pointer;' onclick='showEditWin("+JSON.stringify(row)+")'>填写进度</a><a style='margin-left:5px;cursor:pointer;' onclick='showDetail("+JSON.stringify(row)+")'>详情</a>";
		return html;
	}else{
