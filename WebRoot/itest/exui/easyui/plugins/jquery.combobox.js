...
(function($){
	function getRowIndex(target, value){
		...
		return $.exui.indexOfArray(state.data, state.options.valueField, value);
	}
	...
	function setValues(target, values, remainText){
		...
		if (panel.is(':visible')){
			panel.find('.combobox-item-selected').each(function(){
				...
				if (row){
					if ($.exui.indexOfArray(oldValues, row[opts.valueField]) == -1){
		...
		$.map(oldValues, function(v){
			if ($.exui.indexOfArray(values, v) == -1){
		...
		function findText(value, a){
			var item = $.exui.getArrayItem(a, opts.valueField, value);
	...
	function loadData(target, data, remainText){
		...
		$.exui.forEach(state.data, false, function(row){
			if (row['selected']){
				$.exui.addArrayItem(vv, row[opts.valueField]+'');
			}
		});
		if (opts.multiple){
...
//		param = param || {};
	...
	$.fn.combobox = function(options, param){
		...
		return this.each(function(){
			...
			if (state.options.dict) {
                var dict = state.options.dict;
                var dType = state.options.dictType;
                var dictDefault = {
                    'list': [],
                    'getted': false,
                    'data': {}
                };
                if (!$(document).data('pageCombos')) $(document).data('pageCombos', {});
                if (!$(document).data('pageCombos')[dType + '^' + dict]) $(document).data('pageCombos')[dType + '^' + dict] = dictDefault;
                if ( - 1 === $.inArray($(this), $(document).data('pageCombos')[dType + '^' + dict]['list'])) {
                    $(document).data('pageCombos')[dType + '^' + dict]['list'].push($(this));
                }
                if (!state.options.data && $(document).data('pageCombos')[dType + '^' + dict]['getted'] && $(document).data('pageCombos')[dType + '^' + dict]['data']) {
                	state.options.data = $(document).data('pageCombos')[dType + '^' + dict]['data'];
                }
            }
				...
				if ($.isArray(values)){
					values = $.map(values, function(value){
						if (value && typeof value == 'object'){
							$.exui.addArrayItem(opts.mappingRows, opts.valueField, value);
	...
	var defaultView = {
		render: function(target, container, data){
			...
			COMBOBOX_SERNO++;
			state.itemIdPrefix = '_exui_combobox_i' + COMBOBOX_SERNO;
			state.groupIdPrefix = '_exui_combobox_g' + COMBOBOX_SERNO;		
			state.groups = [];
	...
	$.fn.combobox.defaults = $.extend({}, $.fn.combo.defaults, {
						...
						if (existing){
							if (opts.reversed){
								...
								$.map($(target).combobox('getValues'), function(v){
									var index = $.exui.indexOfArray($(target).combobox('getData'), opts.valueField, v);
									if (index >= 0){
										vv.push(v);
									}
								});
		...
		finder:{
			...
			getGroupEl:function(target, gvalue){
				var state = $.data(target, 'combobox');
				var index = $.exui.indexOfArray(state.groups, 'value', gvalue);
