...
(function($){
	...
	$.parser = {
		...
		plugins:['draggable','droppable','resizable','pagination','tooltip',
				 ...
				 'layout','panel','edatagrid','datagrid','propertygrid','treegrid','datalist','tabs','accordion','window','dialog','form',
				 'ribbon','texteditor','color'
		],
		parse: function(context){
			var aa = [];
			for(var i=0; i<$.parser.plugins.length; i++){
				var name = $.parser.plugins[i];
				var r = $('.exui-' + name, context);
				if (r.length){
					/*if ($.inArray(name, ['datagrid','window'])) {
						r.addClass('hidden');
					}*/
					if (r.hasClass('exui-' + name)) {
						r.removeClass('exui-' + name);
					}
					
					if (r[name]){
						r.each(function(){
							var options = {};
							var s = $.trim($.data(this, 'options'));
							if (s){
								if (s.substring(0, 1) != '{'){
									s = '{' + s + '}';
								}
			...
			if (aa.length && window.exuiloader){
				var names = [];
				for(var i=0; i<aa.length; i++){
					names.push(aa[i].name);
				}
				exuiloader.load(names, function(){
					for(var i=0; i<aa.length; i++){
						var name = aa[i].name;
						var jq = aa[i].jq;
						jq.each(function(){
	...
	$(function(){
		...
		if (!window.exuiloader && $.parser.auto){
			$.parser.parse();
		}
	});
	...
	$.fn._size = function(options, parent){
		if (typeof options == 'string'){
			...
			return this.each(function(){
				...
				if (r1 || r2){
					$(this).addClass('exui-fluid');
				} else {
					$(this).removeClass('exui-fluid');
				}
			});
		}
