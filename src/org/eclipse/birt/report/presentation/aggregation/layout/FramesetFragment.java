...
//		log.setOperId("1289630248984");
		log.setAccessIp("123");
		if(logProducer==null){
			logProducer = (LogProducer)Context.getInstance().getBean("logMessageProducer");		
		}
		//System.out.println("============lgo lso ls  ");
		logProducer.log(log);
	}
	/**
	...
	protected void doPreService(HttpServletRequest request,
		...
		if (ParameterAccessor.PARAM_FORMAT_PDF.equalsIgnoreCase(format)) {
			...
			if (mimeType == null) {
				mimeType = ReportEngineService.getInstance()
						.getMIMEType(format);
			}
		...
		String filename = ParameterAccessor.getExportFilename(new BirtContext(
				request, response), format, emitterId);
		response
				.setHeader(
