...
public class BirtUtility {
	...
	public static void handleOperation(Operation operation,
		...
		for (int i = 0; i < oprands.length; i++) {
			...
			// display text of parameter
				...
				if (ParameterAccessor.PARAM_ISNULLLIST
				...
				if (paramValues == null) {
					...
					for (int i = 0; i < paramValues.size(); i++) {
						Object paramValueObj = DataUtil.validate(paramName,
								...
								bean.getLocale(), bean.getTimeZone(), isLocale);
	...
	public static Exception makeAxisFault(String qName,
		...
		if (exceptions.size() == 1) {
			...
			AxisFault fault = new AxisFault(
					BirtResources
							.getMessage(ResourceConstants.GENERAL_EXCEPTION_MULTIPLE_EXCEPTIONS));
			fault.setFaultCode(new QName(qName));
	...
	public static Map getAppContext(HttpServletRequest request) {
		...
		if (maxCubeRowLevels >= 0)
			context.put(DataEngine.CUBECUSROR_FETCH_LIMIT_ON_ROW_EDGE, Integer
					.valueOf(maxCubeRowLevels));
		...
		if (cubeMemorySize >= 0)
			context.put(DataEngine.IN_MEMORY_CUBE_SIZE, Integer
					.valueOf(cubeMemorySize));
	...
	public static String findTocByName(IReportDocument doc, String name,
		...
		if (timeZone != null) {
			tocTree = doc.getTOCTree(DesignChoiceConstants.FORMAT_TYPE_VIEWER,
					ULocale.forLocale(locale), BirtUtility
							.toICUTimeZone(timeZone));
		} else {
			tocTree = doc.getTOCTree(DesignChoiceConstants.FORMAT_TYPE_VIEWER,
