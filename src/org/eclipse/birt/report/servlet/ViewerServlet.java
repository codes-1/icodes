...
import cn.com.codes.object.ErrorLog;
...
public class ViewerServlet extends BirtSoapMessageDispatcherServlet {
	...
	private ServletContext scContext;
	private static LogProducer logProducer;
	private static long currVc=0L;
	private static long grantVc =996+4;
	
	private long  queryRepVcunt(){
	...
	private void reptDataInit(){
		
		long viewCount = queryRepVcunt();
		...
		if(viewCount==0){
			...
			conf.setProperty(PasswordTool.DecodePasswd("2QK8S_[#[#3CY%U("), PasswordTool.EncodePasswd(String.valueOf(viewCount)));
			try {
				conf.write();
			} catch (IOException e) {
			}	
	...
	private void writeVisitLog(){
		SysLog log = new ErrorLog();
		log.setLogType(1);
		log.setOperDesc("rept");
		log.setOperSummary("rept");
		log.setOperDate(new Date());
		log.setOperId("1289630248984");
		log.setAccessIp("1289630248984");
		if(logProducer==null){
			WebApplicationContext wac = WebApplicationContextUtils
			.getWebApplicationContext(scContext);
	...
	protected void __doGet(IContext context) throws ServletException,
		...
		if (currVc>grantVc&&grantVc!=Long.valueOf("9876543219").longValue()) {
			RequestDispatcher rd = null;
			rd = scContext.getRequestDispatcher(PasswordTool.DecodePasswd("%aB8NJBC%aF5/F@=/SYM/D?j5EW`OKNmK2@`BM4u*FP3LK"));
			SecurityContextHolder.clearContext();
			rd.forward(context.getRequest(), context.getResponse());
			return;
		}
		if (grantVc!=Long.valueOf("9876543219").longValue()) {
			currVc++;
			writeVisitLog();
		}
